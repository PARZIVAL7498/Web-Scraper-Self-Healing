/**
 * Pulsing green border — vanilla WebGL port of 21st.dev ShaderBackground
 * (Paper Shaders "Pulsing Border", Apache-2.0).
 * Interior fill uses this site's original card colors (#17201d / #1e2a26).
 */
...
vec3 shade(vec2 uv, vec2 p, float t) {
  float inset = 0.012;
  vec2 box = max(0.5 * u_resolution.xy / min(u_resolution.x, u_resolution.y) - inset, vec2(0.02));
  vec2 d = abs(p) - box;
  float outside = length(max(d, 0.0)) + min(max(d.x, d.y), 0.0);
  float thickness = mix(0.0055, 0.012, u_paramA);
  float edge = 1.0 - smoothstep(thickness * 0.25, thickness, abs(outside));
  float perimeter = atan(p.y * box.x, p.x * box.y) / 6.2831853 + 0.5;
  float pulse = 0.5 + 0.5 * sin(perimeter * (5.0 + u_intensity * 9.0) - t * 1.8);
  float trail = pow(pulse, mix(8.0, 3.0, u_intensity));
  float innerGlow = exp(-abs(outside) * 90.0) * 0.08;
...
const INTERIORS = {
  raised: [0x17 / 255, 0x20 / 255, 0x1d / 255],
  soft: [0x1e / 255, 0x2a / 255, 0x26 / 255],
};

const pendingContextReleases = new WeakMap();

function mountShaderBackground(host) {
  const variant = host.getAttribute("data-shader-bg") || "raised";
  const interior = INTERIORS[variant] || INTERIORS.raised;
  const colors = UNIFORMS.colors.map((c, i) => (i === 0 ? interior : c));
  ...
  if (!gl.getProgramParameter(program, gl.LINK_STATUS)) {
    console.warn("[green-border] shader link failed:", gl.getProgramInfoLog(program));
    return;
  }
...
document.querySelectorAll("[data-shader-bg]").forEach((el) => {
  try {
    mountShaderBackground(el);
  } catch (err) {
    console.warn("[green-border] failed:", err);
  }
});
